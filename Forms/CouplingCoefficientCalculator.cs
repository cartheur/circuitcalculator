...
namespace CircuitCalculator
{
    public partial class CouplingCoefficientCalculator : Form
    {
        ...
        {
            double result = 0;
            double mutualInductance = Convert.ToDouble(mutualInductanceValueBox.Text);
            double firstInductance = Convert.ToDouble(aInductanceValueBox.Text);
            double secondInductance = Convert.ToDouble(bInductanceValueBox.Text);
            double denominator = Math.Sqrt((firstInductance * secondInductance));
        ...
        {
            double result = 0;
            double Mab = Convert.ToDouble(mutual);
            double La = Convert.ToDouble(firstInd);
            double Lb = Convert.ToDouble(secondInd);
            double sqrt = Math.Sqrt(La * Lb);
            result = Mab / sqrt;
            resultBox.Text = result.ToString();
